...
func (s *MasterStore) ListBySalon(ctx context.Context, salonID string) ([]domain.Master, error) {
	const q = `
SELECT id, salon_id, user_id, full_name, bio, portfolio_url, is_active, created_at
FROM masters
WHERE salon_id = $1
ORDER BY full_name
`
	...
	for rows.Next() {
		var m domain.Master
		var userID, bio, port sql.NullString
		if err := rows.Scan(&m.ID, &m.SalonID, &userID, &m.FullName, &bio, &port, &m.IsActive, &m.CreatedAt); err != nil {
			return nil, err
		}
		if userID.Valid {
...
`
	var m domain.Master
	var userID, bio, port sql.NullString
	err := s.db.QueryRowContext(ctx, q, id).Scan(&m.ID, &m.SalonID, &userID, &m.FullName, &bio, &port, &m.IsActive, &m.CreatedAt)
...
func (s *MasterStore) CreateForSalon(ctx context.Context, m domain.Master) (domain.Master, error) {
	const q = `
INSERT INTO masters (id, salon_id, user_id, full_name, bio, portfolio_url, is_active)
...
WHERE id=$1
RETURNING salon_id, user_id, created_at
`
	...
	err := s.db.QueryRowContext(ctx, q,
		...
		nullIfEmpty(m.PortfolioURL),
		m.IsActive,
	).Scan(&m.SalonID, &userID, &m.CreatedAt)
	if err == sql.ErrNoRows {
